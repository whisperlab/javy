...
async fn main() -> Result<()> {
    let (clang_path, ar_path, sysroot) = if env::var("DOCS_RS").is_ok() {
        ...
        // Because of (1), we need can't use the WASI SDK so instead we use the system llvm.
        // To compile WASI with the system llvm, we need a WASI sysroot and to install a
